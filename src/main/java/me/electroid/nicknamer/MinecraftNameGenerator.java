...
public class MinecraftNameGenerator {
	...
	public String generate(String seed) {
		...
		for (int i = 0; i <= amount; i++) {
			int action = random(8);
			if (action == 0) {
				username = addNumbers(username);
			} else if (action == 1) {
				username = addYear(username);
			} else if (action == 2) {
				username = addRandomUnderscores(username);
			} else if (action == 3) {
				username = addStrategicalUnderscores(username);
			} else if (action == 4) {
				username = addLazyUnderscore(username);
			} else if (action == 5) {
				username = addPhoneticReplacements(username);
			} else if (action == 6) {
				username = addRandomCapitalization(username);
			} else if (action == 7) {
				username = addLogicalCapitalization(username);
			}
		}
